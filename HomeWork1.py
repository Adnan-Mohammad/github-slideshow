import math
def guess_number(max,min,count,mid):
    count=count+1
    if mid>100:
        print("The number can't exceed 100")
        return count
    answer=input("Is it "+str(mid)+" ? (yes/no) ")
    if answer == "no":
      greater=input("Is the number you guess larger than "+str(mid)+" ? (yes/no) ")
      if greater=="yes":
...
while play_more=="yes":
    if play_more == "yes":
        ...
        play_more=input("Would you like to play more? ")
    else:
        play_more="no"
